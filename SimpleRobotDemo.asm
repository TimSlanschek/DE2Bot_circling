...
; 			STORE	Temp
; 			JUMP	Cond
; Cont:		CALL   Abs         ; get abs(currentAngle - Desired Angle)
; 			ADDI   -3
; 
; Back:		JPOS   TurnLoop    ; if angle error > 3, keep checking
...
SensorSet:	LOAD	Mask5	;Enable sensor 5
			OUT		SONAREN	
;Go forward
GoForward:	LOADI	120
			STORE	DVel
...
TooFar:		IN 		DIST5		;Get Theta
			ADDI	-300		;Check if within 300mm
			JNEG	FORWARD		;GO BACK TO LOOP
			IN 		Theta		
			ADDI	5			
			STORE	DTheta
			JUMP	TooFar		
...
;	LOAD	Mask5
;	OUT 	SONAREN
;	LOADI  -10		; initial angle value
;	STORE  DTheta      ; use API to get robot to face 90 degrees
;TurnLoop:
;	IN 		DIST5		; read distance of sonar 5
;	ADDI 	-600		; about 2ft in mm
;	JNEG	InfLoop		; compare, if neg stop
;	IN 		Theta
;	ADDI	-10
;	STORE 	DTheta
