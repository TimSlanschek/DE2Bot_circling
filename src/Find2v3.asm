...
GoForward:	LOADI	220
			...
			ADDI	-100
			JNEG	rtnClauseA
			IN		Theta		; getting angle
			SUB		StartTheta	;
			JZERO	CondAccA		; if angle is same (makes a loop already) jump out
			LOADI	-1
			JUMP	rtnClauseA			
CondAccA:	LOADI	1			
rtnClauseA:	JPOS	GetOut		; if angle is same (makes a loop already) jump out
			;ExitLoop End
...
TooClose:	IN		Theta
			...
			IN		Theta		; getting angle
			SUB		StartTheta	;
			JZERO	CondAccB		; if angle is same (makes a loop already) jump out
			LOADI	-1
			JUMP	rtnClauseB			
CondAccB:	LOADI	1			
rtnClauseB:	JPOS	GetOut		; if angle is same (makes a loop already) jump out
			;ExitLoop End
	
			LOADI	120			;GO FORWARD
			STORE 	Dvel
			IN		Timer
			STORE	TimeElapsed	;START COUNTING 1 SEC
...
TooFar:		IN 		DIST5		;Get Theta
			...
			IN		Theta		; getting angle
			SUB		StartTheta	;
			JZERO	CondAccC		; if angle is same (makes a loop already) jump out
			LOADI	-1
			JUMP	rtnClauseC			
CondAccC:	LOADI	1			
rtnClauseC:	JPOS	GetOut		; if angle is same (makes a loop already) jump out
			;ExitLoop End
			JUMP	TooFar
			LOADI	120
			STORE	Dvel	
			;IN		Timer
			;ADD		Temp		;get collective time
			;STORE	Temp		;store collective time
			;ADDI	-150		; minus time to make a circle
			;JPOS	Getout		; jump out of loop to next reflector
			IN		Timer
			STORE	TimeElapsed	;START COUNTING 1 SEC
