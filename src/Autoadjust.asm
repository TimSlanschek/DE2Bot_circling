...
SeekLoop:				; interrupts would be cleaner here probably, but this should work?
	IN 		DIST0		; read distance of sonar
	ADDI 	-1000		; 1 meter checking distance in all directions (except backwards)
	JNEG	Approach	; compare, if neg: approach object
	IN 		DIST1		
	ADDI 	-1000		
	JNEG	Approach	
	IN 		DIST2		
	...
	JUMP   	SeekLoop    ; keep checking

Approach:				; different one for every sonar because angles are different
						; cleaner solution exists but this is easier
	CALL 	Wait1
	CALL 	Wait1
	CALL 	Wait1
	CALL 	Wait1
	...
	LOADI  	0
	STORE	DVel    	; set driving speed to 0 on both wheels, at least for now

	IN 		DIST0		; read distance of sonar
	ADDI 	-1000		; 1 meter checking distance in all directions (except backwards)
	JPOS	Son1		; compare, if pos: go to next sonar
	CALL	Abs			; gets absolute value of distance difference
	STORE	Temp		; store absolute distance to object to temp, for later when we do edge case checking
	LOADI  	90			; TODO: pos or neg?
	STORE  	DTheta      ; use API to get robot to face towards object
	; facing object now
	; get to 20cm away	: taking the front-most sonars at 12°, the distance to the object has to be 20.44cm for a 
						; 20cm distance from the robot to the object (simple trigonometry)
	JUMP 	CloseIn
Son1:
	IN 		DIST1		
	ADDI 	-1000		
...
Son5:	
	IN 		DIST5		
	ADDI 	-1000
	; JPOS unnecessary here, there is no other sonar
	CALL	Abs
	STORE	Temp
	LOADI	-90
	STORE 	DTheta
...
CloseIn:
	LOAD	FrontSonars
	OUT		SONAREN		; disable side sonars, only front 2 are enabled

	LOAD  	FSlow
	STORE 	DVel		; ahead slow speed

	IN		DIST2
	ADDI	-300		; 204mm is closest integer value to 20.44cm
	JPOS	CloseIn		; if not close enough, keep going
	IN		DIST3
	ADDI	-300
	JPOS	CloseIn
	...
	STORE	DVel
	LOADI	
	JUMP	SensorSet		; jump to the code circling
	
		
SensorSet:	LOAD	&B00110000	;Enable sensor 4 and 5
			OUT		SONAREN	
...
GoForward:	LOADI	220
			...
			IN		DIST5
			OUT		LCD
			ADDI	-220		;SET MIN RANGE
			JNEG	TooClose	;What to do when too close
			IN		DIST5
			ADDI	-350		;SET MAX RANGE
			JPOS	TooFar		;What to do when too far
			IN		Theta		;
			ADDI	-2			
			STORE	DTheta
			JUMP	GoForward
TooClose:	IN		Theta		;Get Theta
			ADDI	5			;TURN 10o to LEFT
			STORE	DTheta		
			LOADI	120			;GO FORWARD
			STORE 	Dvel
			OUT		Timer		;START COUNTING 1 SEC	
TC1:		IN		Timer		;READ Counter
			ADDI	-2			;Check if 0.5 passed
			JPOS	TC2			;IF YES GO ON TO TURNING RIGHT	
			JUMP	TC1			;Continue forward if < 1s
TC2:		LOAD	Zero		
			STORE	Dvel
			IN 		DIST5		;READ DISTANCE SONAR 5
			;ADDI	-400		;READ IF OUTSIDE 400
			;JNEG	TooClose
			;IN 	DIST5		;
			ADDI	-270		;
			JPOS	GoForward	;
			IN		Theta		;READ CURRENT ANGLE
			ADDI	-5			;TURN 5o RIGHT
			STORE	DTheta		;SET DESIRED ANGLE
			JUMP	TC2
				
TooFar:		IN 		DIST5		;Get Theta
			ADDI	-450		;CHECK IF TOOOO FAR AWAY
			JPOS	TF1			
			IN		DIST5
			ADDI	-350		;Check if within 400mm
			JNEG	GoForward	;GO BACK TO LOOP
			IN 		Theta		
			ADDI	-5			;Turn right by 5o			
			STORE	DTheta
			JUMP	TooFar
			LOADI	120
			STORE	Dvel	
			OUT		Timer		;START COUNTING 1 SEC	
TF1:		IN		Timer		;READ Counter
			ADDI	-5			;Check if 0.5 passed
			JPOS	TF2			;IF YES GO ON TO TURNING RIGHT	
			JUMP	TF1			;Continue forward if < 0.5s
TF2:		LOAD	Zero		
			STORE	Dvel
; 			IN 		DIST5		;READ DISTANCE SONAR 5
; 			ADDI	-400		;READ IF OUTSIDE 400
